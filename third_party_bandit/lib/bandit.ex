defmodule Bandit do
  ...
  * `before_payload`: An optional one-argument function called with the payload length after a
    valid frame header is parsed and before payload extraction begins. It may return `:ok` to
    continue, `{:ok, reference}` to continue with a tracked admission, `{:error, reason}` to reject
    the frame, or `{:suspend, reference}` to keep the
    socket passive until `{:bandit_payload_admitted, reference}` arrives. Defaults to `nil`
  * `payload_timeout_ms`: Maximum time for a tracked admitted payload to arrive completely. A
    timeout closes the peer with retryable code 1013. Defaults to 30 seconds
  ...
  * `log_protocol_errors`: How to log protocol errors such as malformed frames. `:short` and
  ...
  @type websocket_options :: [
          ...
          | {:payload_timeout_ms, pos_integer()}
  ...
  @websocket_keys ~w(enabled max_frame_size max_fragmented_message_size before_payload payload_timeout_ms validate_text_frames compress deflate_options max_inflate_ratio primitive_ops_module log_protocol_errors)a
