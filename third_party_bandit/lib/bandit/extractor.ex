defmodule Bandit.Extractor do
  ...
  @type t :: %__MODULE__{
          ...
          before_payload:
            nil
            | (non_neg_integer() ->
                 :ok | {:ok, reference()} | {:error, term()} | {:suspend, reference()}),
          pending_admission: nil | {reference(), non_neg_integer(), non_neg_integer()},
          payload_admission: nil | reference(),
          completed_payload_admission: nil | reference()
        }
  ...
  defstruct header: <<>>,
            ...
            payload_admission: nil,
            completed_payload_admission: nil
  ...
  def new(frame_parser, primitive_ops_module, opts) do
    ...
    %__MODULE__{
      ...
      before_payload: Keyword.get(opts, :before_payload)
    }
  end
  ...
  def pop_frame(%__MODULE__{mode: :header_parsing} = state) do
    case state.frame_parser.header_and_payload_length(state.header, state.max_frame_size) do
      {:ok, {header_length, required_length}} ->
        case admit_payload(state.before_payload, required_length) do
          :ok ->
            state
            |> transition_to_payload_parsing(header_length, required_length, nil)
            |> pop_frame()

          {:ok, reference} ->
            state
            |> transition_to_payload_parsing(header_length, required_length, reference)
            |> pop_frame()
          ...
          {:suspend, reference} ->
            state = %{state | pending_admission: {reference, header_length, required_length}}
            {state, {:suspend, reference}}
        end
  ...
  @spec resume_payload(t(), reference()) :: t()
  def resume_payload(
        %__MODULE__{pending_admission: {reference, header_length, required_length}} = state,
        reference
      ) do
    state
    |> Map.put(:pending_admission, nil)
    |> transition_to_payload_parsing(header_length, required_length, reference)
  end

  def payload_admission(%__MODULE__{payload_admission: reference}), do: reference

  def take_completed_payload_admission(
        %__MODULE__{completed_payload_admission: reference} = state
      ) do
    {reference, %{state | completed_payload_admission: nil}}
  end

  defp admit_payload(nil, _length), do: :ok
  defp admit_payload(callback, length), do: callback.(length)

  defp transition_to_payload_parsing(state, header_length, required_length, admission) do
    ...
    |> Map.put(:payload_admission, admission)
  end
  ...
  defp transition_to_header_parsing(state, rest) do
    ...
    |> Map.put(:completed_payload_admission, state.payload_admission)
    |> Map.put(:payload_admission, nil)
  end
end
