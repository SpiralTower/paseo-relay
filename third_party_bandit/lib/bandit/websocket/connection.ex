defmodule Bandit.WebSocket.Connection do
  ...
  def handle_frame(frame, socket, %{fragment_frame: fragment_frame} = connection)
    ...
    case frame do
      %Frame.Continuation{fin: true} = frame ->
        ...
        if oversize_message?(fragment_size, connection.opts) do
        ...
        if frame_size == 0 do
          ...
          if oversize_message?(fragment_size, connection.opts) do
            ...
            {:continue,
             %{
               connection
               | fragment_frame: %{connection.fragment_frame | data: data},
  ...
  defp oversize_message?(size, opts) do
    case Keyword.get(opts, :max_fragmented_message_size, 8_000_000) do
      0 -> false
      max_fragmented_message_size -> size > max_fragmented_message_size
    end
  end
  ...
  def handle_payload_timeout(socket, connection),
    do:
      do_error(
        {1013, "Payload completion timeout"},
        :payload_completion_timeout,
        socket,
        connection
      )
  ...
  defp handle_continutation(continutation, socket, connection) do
    case continutation do
      {:suspend, websock_state} ->
        {:suspend, %{connection | websock_state: websock_state}}

      {:resume, websock_state} ->
        {:resume, %{connection | websock_state: websock_state}}
